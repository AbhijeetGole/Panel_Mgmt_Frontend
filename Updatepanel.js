...
const Updatepanel = ({ token }) => {
  ...
  const handleget1 = async (e) => {
    e.preventDefault();
    try {
      await Authservice.getPanelId(panelId).then(
        (data) => {
          if (data.result)
            navigate("/Update_panel2")
          else
                ...
                <div className="card-body">
                  ...
                  <form onSubmit={handleget1}>
                    ...
                    <div className="d-grid gap-2 d-md-flex justify-content-md-end">
                      ...
                      <button className="btn btn-primary" type="button" onClick={handleget1}>Update</button>
                    </div>
                  </form>
                </div>
              </div>
